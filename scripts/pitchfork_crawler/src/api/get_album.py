...
def get_album(album_id, credentials, abort_on_error=True, global_ident=4):
    with yaspin(
        side="right",
        text=Fore.YELLOW
        + " " * global_ident
        + "GET https://api.spotify.com/v1/albums/{}".format(album_id)
        + Style.RESET_ALL,
        color="yellow",
    ) as spinner:
        res = requests_retry_session().get(
            "https://api.spotify.com/v1/albums/{}".format(album_id),
